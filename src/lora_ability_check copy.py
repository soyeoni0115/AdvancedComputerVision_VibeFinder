...
TEST_PATH= BASE_DIR / "data" / "processed" / "valid_unseen"
...
def load_images(image_paths, labels, max_samples=300):
    images = []
    valid_labels = []
    ...
    for path_str, label in zip(image_paths, labels):
        ...
        # 🔥 파일명만 추출해서 raw 폴더에 붙이기
        fixed_path = BASE_DIR / "data" / "raw" / path.name

        if not fixed_path.exists():
            print(f"[경로 없음] {fixed_path}")
            continue

        try:
            img = Image.open(fixed_path).convert("RGB")
            images.append(img)
            valid_labels.append(label)
        except Exception as e:
            print(f"[이미지 로딩 실패] {fixed_path} | {e}")
            continue
    ...
    if len(images) == 0:
        raise RuntimeError("이미지 0개 → 경로 문제 확정")
...
if __name__ == "__main__":
    ...
    print("이미지 로딩 중...")
    images, valid_labels = load_images(image_paths,labels)
    print("images 개수:", len(images))
    print("labels 개수:", len(valid_labels))
    print("class 개수:", len(class_names))
    print("class_names:", class_names[:10])
